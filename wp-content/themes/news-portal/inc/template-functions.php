...
if( ! function_exists( 'news_portal_dynamic_styles' ) ) :
    function news_portal_dynamic_styles() {
        ...
        $news_portal_theme_hover_color = news_portal_hover_color( '#fefe1af5', '-0' );
        ...
        foreach( $get_categories as $category ){
            ...
            $cat_color = get_theme_mod( 'news_portal_category_color_'.strtolower( $category->name ), '#e80902' );

            $cat_hover_color = news_portal_hover_color( $cat_color, '-50' );
            $cat_id = $category->term_id;
            
            if( !empty( $cat_color ) ) {
                $output_css .= ".category-button.np-cat-". esc_attr( $cat_id ) ." a { background: #ffff00c2;color: ". esc_attr( $news_portal_theme_color ) ." !important;font-size: 15px;}\n";
